...
class AgentController extends Controller
{
    ...
    $order = Order::getCurrentValidOrder($agentId);

    if (!$order) {
        return redirect()->route('agent_payment')->with('error', 'You have not purchased any package yet or all your packages have expired. Please purchase a package to see properties.');
    }
    ...
    if (!$order) {
        return redirect()->route('agent_payment')->with('error', 'You have not purchased any package yet or all your packages have expired. Please purchase a package to create properties.');
    }
    if ($order->package->allowed_properties != -1 &&
    $order->package->allowed_properties <= Property::where('agent_id', $agentId)
    ...
    if ($request->is_featured == 'Yes') {
        $featuredCount = Property::where('agent_id', $agentId)
                                  ->where('order_id', $order->id)
                                  ->where('is_featured', 'Yes')
                                  ->count();

        if ($order->package->allowed_featured_properties != -1 &&
            $order->package->allowed_featured_properties <= $featuredCount) {
            return redirect()->back()->with('error', 'You have reached the maximum number of featured properties allowed in your package.');
        }
    }

    $propertyCount = Property::where('agent_id', $agentId)
                              ->where('order_id', $order->id)
                              ->count();

    if ($order->package->allowed_properties != -1 &&
        $order->package->allowed_properties <= $propertyCount) {
        return redirect()->route('agent_payment')->with('error', 'You have reached the maximum number of properties allowed in your package.');
    }

    // Proceed with saving the property (your saving logic here)
        ...
        $property->order_id = $order->id; // ✅ Store order_id to link with current package
    ...
    public function property_photo_gallery_store(Request $request, $id)
{
    ...
    // ✅ Check photo limit, allow unlimited if -1
    $photoLimit = $order->package->allowed_photos;
    $currentPhotos = PropertyPhoto::where('property_id', $id)->count();

    if ($photoLimit != -1 && $photoLimit <= $currentPhotos) {
...
{
    // ✅ Check if the agent has an active package
    $order = Order::where('agent_id', Auth::guard('agent')->user()->id)
    ...
    // ✅ Check if video upload is within allowed limit (support -1 = unlimited)
    $videoLimit = $order->package->allowed_videos;
    $currentVideos = PropertyVideo::where('property_id', $id)->count();

    if ($videoLimit != -1 && $videoLimit <= $currentVideos) {
    ...
    // ✅ Ensure property belongs to the logged-in agent
    ...
    // ✅ Validate that the video field is filled
    $request->validate([
        'video' => ['required'],
    ]);

    // ✅ Extract YouTube video ID from full URL or short URL
    $fullLink = $request->video;
    parse_str(parse_url($fullLink, PHP_URL_QUERY), $urlParams);
    $videoId = $urlParams['v'] ?? null;

    if (!$videoId) {
        // For youtu.be or embed links
        $path = parse_url($fullLink, PHP_URL_PATH);
        $segments = explode('/', trim($path, '/'));
        $videoId = end($segments);
    }

    // ✅ Final validation of video ID format
